#!/usr/bin/env python3
"""Fuse converted T2S/VITS modules and baked voice conditioning into one CPU graph."""
...
class AcousticPipeline(torch.nn.Module):
    def __init__(self, t2s, vits, conditioning: dict[str, torch.Tensor], eos: int = 1024, max_tokens: int = 1500):
        super().__init__()
        self.t2s = t2s
        self.vits = vits
        self.eos = eos
    ...
    def forward(
        ...
        y_len, y, xy_pos, k_cache, v_cache = self.t2s.pre_infer(
            self.prompt_semantic, self.prompt_phone_ids, text_seq, self.prompt_bert, text_bert,
...
def main() -> None:
    ...
    module=AcousticPipeline(torch.jit.load(root/'t2s.pt',map_location='cpu'),torch.jit.load(root/'vits.pt',map_location='cpu'),conditioning)
    module.eval(); scripted=torch.jit.script(module)
    # Inline called submodules and remove exported-but-unreachable methods such as reference STFT.
